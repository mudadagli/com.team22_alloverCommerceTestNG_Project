...
public class TC_008 extends TestBaseReport {
    ...
    //Test Case: Clear cart ile rakamlar silinebilmeli
    @Test
    public void test008() throws InterruptedException {
        ...
        //Clean cart ' tiklanir
        jse.executeScript("arguments[0].scrollIntoView(true);",ahmetPage.clearCartButton);
        jse.executeScript("arguments[0].click();", ahmetPage.clearCartButton);
        extentTest.info("Clean cart'a tıklandı");

        //Your cart is currently empty. mesajinin gorundugu test edilir
        Assert.assertTrue(ahmetPage.cartEmptyLabel.isDisplayed());
        extentTest.pass("Your cart is currently empty. mesajinin ekranda oldugu goruldu");
    }
