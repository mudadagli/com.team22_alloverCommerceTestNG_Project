...
public class TC_004 extends TestBaseReport {
    ...
    //Test Case: Tum address bilgileri tam olarak girilmeli
    @Test
    public void test004() throws InterruptedException {
        ...
        //Tum adres bilgilerinin girildigi dogrulanir
        jse.executeScript("arguments[0].scrollIntoView(true);",ahmetPage.shipCountryDropBox);
        jse.executeScript("arguments[0].click();", ahmetPage.shipCountryDropBox);
        Select country=new Select(ahmetPage.shipCountryDropBox);
        country.selectByValue("TR");
        Select city=new Select(ahmetPage.shipCityDropBox);
        city.selectByValue("TR46");
        ahmetPage.shipStateDropBox.clear();
        ahmetPage.shipStateDropBox.sendKeys("Afsin");
        ahmetPage.shipPostCodeBox.clear();
        ahmetPage.shipPostCodeBox.sendKeys("46500");
        ahmetPage.updateTotalButton.click();
        String expectedAdres="46500 Afsin Kahramanmaraş, Turkey";
        String actualAdres=ahmetPage.shipAdresText.getText();
        Assert.assertEquals(actualAdres,expectedAdres);
        extentTest.pass("Tum adres bilgilerinin girildigi dogrulandi");
