...
public class TC_005 extends TestBaseReport {
    ...
    //Test Case: Billing bilgileri tam olarak eklenmeli ve place order diyerek order yapılmalı
    @Test
    public void test005() throws InterruptedException {
        ...
        //Checkout butonu tiklanir
        ahmetPage.cartCheckOutLink.click();
        extentTest.info("Checkout butonu tiklandı");

        //Tum billing bilgileri girilir
        ahmetPage.billFirstNameBox.clear();
        ahmetPage.billFirstNameBox.sendKeys("enes");
        ahmetPage.billLastNameBox.clear();
        ahmetPage.billLastNameBox.sendKeys("palta");
        ahmetPage.billStreetBox.clear();
        ahmetPage.billStreetBox.sendKeys("atlas");
        ahmetPage.billPostCodeBox.clear();
        ahmetPage.billPostCodeBox.sendKeys("46500");
        ahmetPage.billTownCityBox.clear();
        ahmetPage.billTownCityBox.sendKeys("Afsin");
        ahmetPage.billPhoneBox.clear();
        ahmetPage.billPhoneBox.sendKeys("3445112121");
        ahmetPage.billEmailBox.clear();
        ahmetPage.billEmailBox.sendKeys("a.palta1314@hotmail.com");
        Thread.sleep(2000);
        jse.executeScript("arguments[0].click();", ahmetPage.billPaymentRadioButton);
        Thread.sleep(2000);

        jse.executeScript("arguments[0].click();", ahmetPage.billPlaceOrderButton);
        Thread.sleep(2000);
        extentTest.info("Tum billing bilgileri girildi ve Place Orders butonuna tiklandi");

        String expectedMesaj="Thank you. Your order has been received.";
        String actualMesaj=ahmetPage.thankYouText.getText();
        Assert.assertEquals(actualMesaj,expectedMesaj);
        extentTest.pass("Thank you. Your order has been received. mesajinin alindigi goruldu");
